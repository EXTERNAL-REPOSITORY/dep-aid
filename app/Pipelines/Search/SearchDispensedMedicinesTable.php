        ...
        if (request()->has('search')) {
            $query->where(function ($query) {
                $query->orWhere('dispensed_medicines.patient_name', 'LIKE', "%" . request('search') . "%")
                    ...
                    ->orWhere('patient_form.firstname', 'LIKE', "%" . request('search') . "%")
                    ->orWhere('patient_form.middlename', 'LIKE', "%" . request('search') . "%")
                    ->orWhere('patient_form.lastname', 'LIKE', "%" . request('search') . "%")
